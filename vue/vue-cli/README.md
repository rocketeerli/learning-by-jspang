...
改造 `methods` 方法：
    ...
    methods:{
        ...
        ...mapActions(['addAction', 'subAction'])
    }
...
### 在 `vuex/store.js` 中声明模块组

	const moduleA = {
		state, mutations, getters, actions
	}

### 修改 `Vuex.Stroe` 里的值进行导出
...
### 在模板中使用
	...
	<h3>Vuex 写法： {{$store.state.a.count}}</h3>

### 简单的方法引入

模板：

	<h3>computed: {{count}}</h3>
...
插件功能还是很强大的，但是感觉有点跟之前学的 `Vue` 基础脱轨了，一些没咋用的基础知识又快遗忘了，我这脑瓜子...
